...
function db_get_articles($link){ // complete with the session limit values
	...
	while ($data = $result -> fetch()){
		if(array_key_exists($data['tag'], $facet)){
			if($request == "")
				$request = sprintf("SELECT * FROM (SELECT *, pp_article.date AS a_date FROM pp_article WHERE id_category = %s ) AS a ",$data['id']);
			else
				$request .= sprintf(" UNION SELECT * FROM (SELECT *, pp_article.date AS a_date FROM pp_article WHERE id_category = %s ) AS %s", $data['id'], $range[$i]);
		}	
		$i++;
	}
